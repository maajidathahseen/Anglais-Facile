...
Bienvenue dans ce projet AnglaisFacile !

je creer une site qui parle de mon centre de formation AnglaisFacile.
...
```
https://github.com/maajidathahseen/Anglais-Facile.git
```
...
```bash
git clone https://github.com/maajidathahseen/Anglais-Facile.git
```
...
De plus, vous pouvez ajouter les champs du formulaire un par un selon vos besoins
(ex : nom, prénom, email, message, téléphone, sujet…).
Wagtail vous permet de créer un formulaire personnalisé directement depuis l’interface d’administration, sans écrire de code : il suffit d’aller dans l’onglet “Form fields” / “Champs du formulaire” et d’ajouter les champs souhaités.
...
Car ce sont les champs connectés au CMS.
Vous pouvez par contre les déplacer, styliser, mettre dans des colonnes, etc.
...
Ce template est fait pour être personnalisé facilement.
Vous pouvez :
...
La page **Contact** peut être configurée pour envoyer un email via Gmail lorsque quelqu’un soumet le formulaire.
...
1. Connectez-vous à votre compte Google (celui qui enverra les emails).
2. Allez dans **Gestion du compte Google → Sécurité**.
3. Activez la **validation en deux étapes** (2FA) si ce n’est pas déjà fait.
4. Ensuite, allez dans **Mots de passe d’application**.
5. Créez un nouveau mot de passe :

   * Application : **Mail**
   * Appareil : **Autre (Django)** par exemple
6. Google vous donne un mot de passe du type :

   ```text
   abcd efgh ijkl mnop
   ```

   ➜ Gardez-le précieusement : c’est ce que nous utiliserons dans le projet
   (⚠️ sans les espaces dans le `.env`).
